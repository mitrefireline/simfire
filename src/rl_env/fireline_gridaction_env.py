...
class FireLineGridEnv():
    ...
    def _reset_state(self):
        ...
        print(np.all(self.terrain.elevations >= -500))
        where = np.where(self.terrain.elevations < -500)
        print(self.terrain.elevations[where])
        exit()
...
class RLGridEnv(gym.Env):
    ...
    '''
    def __init__(self, simulation: FireLineGridEnv):
        '''
