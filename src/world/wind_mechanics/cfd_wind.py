...
class Fluid():
    def __init__(self, n: int, iterations: int, scale: int, dt: float, diffusion: float, viscosity: float, terrain: np.ndarray) -> None:
        ...
        for i in range(0, self.N):
            for j in range(0, self.N):
                ...
                if (not (abs(vx) < 0.1 and abs(vy) ) <= 0.1):
                    ...
                    if meanval > 255:
                        meanval = 255
                    pygame.draw.line(surface, [meanval,meanval,meanval,meanval], [x,y], [x + vx, y + vy])
        pygame.display.flip()
...
def lin_solve(b: int, x: np.ndarray, x0: np.ndarray, a: float, c: float, itr: int, N: int):
    cRecip = 1.0 / c
...
def diffuse(b: int, x: np.ndarray, x0: np.ndarray, diff: float, dt: float, itr: int, N: int):
    a = dt * diff * (N - 2) * (N - 2)
    lin_solve(b, x, x0, a, 1+6 * a, itr, N)

# Incompressible object, clean up stage for each cell
def project(velocX: np.ndarray, velocY: np.ndarray, p: np.ndarray, div: np.ndarray, itr: int, N: int):
    for j in range(1, N - 1):
        for i in range(1, N - 1):
...
# then fllows that velocity back in time and sees where it lands.  takes weighted average of
# cells around the spot where it lands, then applies that value to current cell
def advect(b: int, d: np.ndarray, d0: np.ndarray, velocX: np.ndarray, velocY: np.ndarray, dt: float, N: int):
    dtx = dt * (N - 2)
    dty = dt * (N - 2)
